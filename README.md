...
- x integrate interp model on this repo
- [ ] integrate sync-train method part
- [ ] integrate label-train method part
...
- Training an agent to stay alongside other cars in Enduro using _human_ preferences.
...
For example, to train `MovingDotNoFrameskip-v0` using _synthetic_ preferences:
...
To train Pong using _synthetic_ preferences:
...
To train Enduro (a modified version with a time limit so the weather doesn't change, which the paper notes can confuse the reward predictor) using _human_ preferences:
...
- The paper selects video clips to show the user based on predicted reward
  ...
  any ensembling. (Ensembling code _is_ implemented in
