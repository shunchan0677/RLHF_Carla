...
def sample_actions(mean, std):
    ...
    # Clip sampled actions

def sample(logits):
    mean_new = logits[:, 0:2]
    std_new = tf.math.softplus(logits[:, 2:4])
    sampled_actions = sample_actions(mean_new, std_new)
    return clip_actions(sampled_actions)

def sample_(logits):
    # 出力ノードの範囲を調整
    output_A = tf.tanh(logits[:, 0:1])
    output_B = tf.sigmoid(logits[:, 1:2])
    output_C = tf.sigmoid(logits[:, 2:3])
